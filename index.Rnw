...
Graphics provide the opportunity to understand statistical data at an intuitive level: \dc{if the relationship between two variables is not adequately summarized by the numerical statistics} we can gain more information by considering a simple scatter plot than we might obtain from an entire day of staring at the numbers. 
