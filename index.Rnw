...
\section{Estimation using null choices in a one target lineup}\label{sec:vis-estimation-alpha}
...
\subsection{Estimation using additional Rorschachs}\label{sec:rorschach-est-alpha}
...
We can see from \autoref{fig:vis-p-val-sensitivity-initial} that the proposed method will generate $p$-values which are more moderate than the simpler Binomial model that does not involve the extra step of estimating $\alpha$. In the next section, we explore the practical implications of this method by examining actual lineups which were tested experimentally and the visual $p$-values calculated using each method. \svp{XXX If we've already said that the Binomial model is only for Scenario 1, then do we need to compare the two p-values? XXX}
