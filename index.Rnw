...
This is particularly true for the marginal \dc{BB} model, where the equivalent of $(m-1)\alpha$ pseudo-observations are added. 
...
A \svp{discussion} of this property \svp{and its application to visual inference} is provided in \Cref{neutrality}.
...
\begin{description}
\item[\hh{Identify the number of $c$-interesting null panels in the lineup of interest.}] \hh{Consider \Cref{fig:alpha-sim-curve}. Generally, this figure needs to be adjusted to match the number of null panels,   $m_0$,  and the corresponding number of null panel selections, $K_0$. } \svp{The \texttt{alpha\_from\_data\_lineup()} function in the \texttt{vinference} package (available at https://github.com/heike/vinference) can be used to create this plot given values of $c$, $m_0$, and $K_0$.}
...
This estimation method \svp{requires the following steps}:
\begin{description}
...
\svp{The \texttt{alpha\_from\_null\_lineup()} function in the \texttt{vinference} package (available at https://github.com/heike/vinference) can be used to estimate $\hat\alpha$ given values of $\overline Z_c$, $c$, $m_0$, and $K_0$.}
...
\dc{An example is} described in \citet{vanderplas:2017}, \dc{where the generated null plots occasionally lacked important features critical to the study that made them inadvertently visually different.}
...
This suggests that $p$-values generated using the \dc{BB} mixture model are better calibrated than those generated by the Binomial model.
