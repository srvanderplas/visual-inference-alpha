...
Under the Beta-Binomial mixture model the visual $p$-value for the data shown in lineup in \autoref{fig:example-plot} is \Sexpr{round(new_vispval,2)} based on $\hat{\alpha} = 0.174$; while this is still significant at the 0.05 level, it is not nearly as small as the $p$-value computed under the Binomial model.
