...
While it is generally possible to \dc{use maximum likelihood to} estimate $\alpha$ \citep{sirt, minka} directly based on proportions observed from panel selections in a lineup, these approaches fail because of the large number of observed zeroes. \dc{Zeroes are the consequence of the tendency for participants to selecting a small subset of interesting null panels, leaving many null panels with no selections, espcially under scenario 3.}
...
\dc{Instead, we} propose a method for \emph{visual} estimation of $\alpha$ \dc{accommodating a prevalence of zero-count values,
and also} a \dc{numerical} method for estimating $\alpha$ based on the results of Rorschach lineup based testing. \dc{These are described here.}
