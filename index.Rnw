...
If the visual method of $\alpha$ estimation is inconclusive, or there are insufficient null panel selections to estimate $\alpha$ visually, \autoref{eqn:differentPanels} can be used to generate a more precise estimate of $\alpha$ with a secondary study consisting of one or more Rorschach lineups evaluated $K$ times each.
...
When only one null plot in a Rorschach lineup is visually interesting, we encounter two problems.
