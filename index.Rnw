...
\begin{enumerate}
\item \dc{Cut null panel selection activity into four levels as shown in \autoref{fig:alpha-sim-curve}}, using a visual display with both simulated values and the expected value for the situation ($c = 1, m_0 = 19, K_0 = 30$). %, determine ranges of interesting null panel counts which correspond to about \dc{four} levels of null panel selection activity. 
These levels are 2-3, 4-5, 6-7, and 8+ panels with more than $c=1$ selection.
...
\item Using the simulated selected panel counts, \dc{on the vertical axis locate} the band which is most consistent with the number of null panels selected \dc{with} more than $c$ times in the lineup. For instance, if there were three null panels with more than $c$ selections, we would use a range of 2-3 panels.
