...
As seen in \Cref{fig:vis-p-val-sensitivity-initial} the Beta Binomial model converges  for $\alpha \rightarrow\infty$ asymptotically to the Binomial model (with $\theta = 1/m$) proposed in \citet{majumder2013validation}.
