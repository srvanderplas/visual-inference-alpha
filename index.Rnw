...
In order to test whether a chart shows a visually significant result, we can use the same \dc{machinery} used by randomization tests: construct a \dc{randomization} \hh{XXX not all nulls are  randomization based - how about "construct a method to generate data consistent with the null hypothesis, such as using randomization or drawing samples from a model "} method consistent with the null hypothesis, generate many copies of the test statistic (in this case, the plot), and see where the \dc{observed} statistic falls in the distribution of artificially generated quantities. \dc{The conceptual framework was described in \citet{buja:2009}), who introduced two protocols: a Rorschach and a lineup. Figure \ref{typesoflineups} illustrates these. In a Rorschach all plots are null plots, and the purpose is to purely digest the variation that migh occur even if the data is generated only by the null mechanism.}
An assembly of several null plots with a target (or data) plot is called a \emph{lineup}, after the criminal procedure of the same name where suspects are lined up for the victim to try to spot if one is the guilty perpetrator of the crime. Typically, lineups are composed of $m-1$ "null" plots (generated under the null hypothesis) and one data plot containing the \dc{observed} data. \dc{We will call this a one target lineup because \citet{vanderplas:2017} introduced the two-target lineups, which can be used to test the visual salience of two competing effects. Technically the Rorschach could also be considered a lineup, of all innocent suspects. And the two target lineup has two guilty suspects.}
...
During this evaluation process, the user selects one or more plots from the lineup which are ``different" in some way, \dc{which may or may not be prescribed}.
...
If multiple individuals select the plot generated from the data rather than the null plots, the visual statistic is likely to be \dc{significantly different from that specified by the null generating mechanism} (\dc{as described in \citet{majumder2013validation} for the infrastructure}).
% TODO: add lineup here

\svp{\citet{majumder2013validation} also} \dc{proposed an approach for calculating a $p$-value from the results of lineup evaluations of multiple subjects that uses the binomial distribution.} \dc{This approach is simple, and a reasonably good approximation for assessing the significcanc eeof structure in a plot, but it fails to account for different ways of applying a llineup protocol for multiple observers.}
...
In addition to different types of lineups, there are different \dc{ways that an analyst can run a lineup experiment.} These are detailed below and also illustrated in Figure \ref{scenarios}.
...
<<scenarios, eval = T, echo=FALSE, out.width='\\textwidth', fig.width=12, fig.height=5, fig.cap = "Illustrations of scenarios 1 and 2. Left column shows what three different subjects would see under scenario 1, and the right column shows what three different subjects would see under scenario 2. In scenario 1, all nulls and data plots are different, while in scenario 2 only the nulls differ.  (Scenario 3 would have all three lineups the same.) The location of the data plot may vary from one subject t another. Need to use patchwork maybe to get better titles.">>=
set.seed(2020)
df <- tibble(x=runif(50)) %>%
  mutate(y1 = -4*(x-0.5) + rnorm(50)) %>%
  mutate(y2 = ifelse(x<0.5, (x-0.5) + 5 + rnorm(50), 
                     (x-0.5) - 5 + rnorm(50)))
p1 <- ggplot(lineup(null_permute('y1'), df, n=5), aes(x, y1)) +
  geom_point() +
  facet_wrap(~ .sample, ncol=5) +
  theme(aspect.ratio=1, 
        axis.ticks = element_blank(), 
...
p2 <- ggplot(lineup(null_permute('y1'), df, n=5), aes(x, y1)) +
  ...
  theme(aspect.ratio=1, 
        ...
        axis.text = element_blank()) +
  xlab("") + ylab("") 
p3 <- ggplot(lineup(null_permute('y1'), df, n=5), aes(x, y1)) +
  geom_point() +
  facet_wrap(~ .sample, ncol=5) +
  theme(aspect.ratio=1, 
        axis.ticks = element_blank(), 
...
df <- tibble(x=runif(50)) %>%
  ...
  mutate(y2 = ifelse(x<0.5, (x-0.5) + 5 + rnorm(50), 
                     (x-0.5) - 5 + rnorm(50)))
p4 <- ggplot(lineup(null_permute('y1'), df, n=5), aes(x, y1)) +
  geom_point() +
  facet_wrap(~ .sample, ncol=5) +
  theme(aspect.ratio=1, 
        axis.ticks = element_blank(), 
...
df <- tibble(x=runif(50)) %>%
  ...
  mutate(y2 = ifelse(x<0.5, (x-0.5) + 5 + rnorm(50), 
                     (x-0.5) - 5 + rnorm(50)))
p5 <- ggplot(lineup(null_permute('y1'), df, n=5), aes(x, y1)) +
  geom_point() +
  facet_wrap(~ .sample, ncol=5) +
  theme(aspect.ratio=1, 
        axis.ticks = element_blank(), 
...
df <- tibble(x=runif(50)) %>%
  ...
  mutate(y2 = ifelse(x<0.5, (x-0.5) + 5 + rnorm(50), 
                     (x-0.5) - 5 + rnorm(50)))
p6 <- ggplot(lineup(null_permute('y1'), df, n=5), aes(x, y1)) +
  geom_point() +
  facet_wrap(~ .sample, ncol=5) +
  theme(aspect.ratio=1, 
        axis.ticks = element_blank(), 
        axis.text = element_blank()) +
  xlab("") + ylab("") 
grid.arrange(p4, p1, p5, p2, p6, p3, ncol=2)
@
...
<<example-plot, dependson = "example-setup", echo = F, fig.width = 5, fig.height = 5, out.width = ".495\\textwidth", fig.cap = "A lineup designed to test the utility of boxplots for detecting distributional differences. Selection counts are provided in light grey in the lower right corner of each panel which was selected at least once.", fig.subcap = c("A one-target lineup (data is in panel 3+5).", "A Rorschach version of the same lineup (not evaluated experimentally).") >>=
ggplot(exdat, aes(x = group, y = vals, fill = factor(group))) + geom_boxplot() + 
  facet_wrap(~.sample) + 
  theme_bw() + 
