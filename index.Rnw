...
\svp{XXX todo XXX reformat this picture so that it has a 3x3 grid of alpha values (theoretical) with selection probabilities $\theta_i$; add in a 3x1 grid of turk experiment selection probabilities to provide evidence that not all null panels are equally interesting.}
