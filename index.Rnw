...
From \autoref{fig:prior-predictive}, we know that our observations are most consistent with $\alpha << 1$. 
...
In the theoretical Multinomial-Dirichlet model proposed in this paper, the number of panels which could be expected to be visually different in a generalized lineup is a function of $\alpha$, the hyperparameter in \autoref{eqn:dirichlet-pdf} and \autoref{eqn:full-model-specification} \svp{(a derivation of this expectation is provided in \autoref{app:expected}).} 
