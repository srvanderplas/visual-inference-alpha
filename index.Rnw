...
This abstract plot definition \svp{allows us to explicitly declare the null hypothesis by specifying the relationship between the variables and the spatial elements of the plot.} 
% to come from under the rug (implicit), be explicitly declared. 
For examine, the scatterplots shown in the one target lineups in Figure \ref{fig:typesoflineups}b, are defined as mapping variable 1 to the $x$ axis and variable 2 to the $y$ axis, with the intent to study association between the two variables.

With numerical statistics, there is a natural ordering to the computed numerical quantities. 
With plots, we must run each statistic through another process: evaluation by the visual system, or a facsimile thereof, in order to evaluate significance. 
