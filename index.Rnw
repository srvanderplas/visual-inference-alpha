...
\svp{
In the case where a tested lineup has selections which overwhelmingly favor the data plot, we have a problem: the lineup is likely significant (based on the overwhelming evidence that the data plot attracts the most visual interest), but we cannot estimate $\alpha$ because there are insufficient null panel selections. In this case, estimation of $\alpha$ will depend on creation of a Rorschach lineup (that is, a lineup consisting entirely of plots generated under the null hypothesis). Having this Rorschach lineup evaluated separately will provide an estimate of $\alpha$ which is unaffected by the data plot; then, a p-value for the data lineup can be computed using the $\alpha$ estimated from the Rorschach lineup.
}
...
\begin{enumerate}
\item Simulate, for each potential value of $\alpha$, the number of panels selected by a number of participants exceeding an interest threshold $c$ (in \autoref{fig:alpha-sim-curve}, $K/m = 30/20 = 1.5 = c$). 
...
In \autoref{fig:alpha-sim-curve}, the bands are approximately 2 panels in width; for $K=30$ this provides a balance between level of detail and our ability to estimate the number of visually interesting null plots.
...
sim_interesting_panels <- function(c = m/K, N_points = 10, m = 19, K = 30, point_avg_sims = 10, alphas = alphas) {
  ...
  df2 <- df %>%
    mutate(point_num = floor(rep/N_points)) %>%
    group_by(alpha, point_num) %>%
    summarize(n_interesting = mean(n_interesting))
}

exp_average <- alphas %>% map_dbl(number_panels, c = interest_threshold)
