...
As seen in \Cref{fig:vis-p-val-sensitivity-initial} the Beta Binomial model converges  for $\alpha \rightarrow\infty$ asymptotically to the Binomial model (with $\theta = 1/m$) proposed in \citet{majumder2013validation}; \hh{see \autoref{eq:binolimit} for a derivation}.

<<vis-p-val-sensitivity-initial, fig.cap = "Sensitivity of visual p-value to selection of $\\alpha$ under the marginal beta-binomial model.  Corresponding values for the binomial model are shown on the right side of the plot; as $\\alpha \\rightarrow\\infty$, the beta-binomial $p$-values converge to the binomial model $p$-value. \\hh{For different values of $\\alpha$ different number of data panel identifications are needed to achieve significance: 5 or more identifications are needed for significance at a level of 0.05 under the Binomial model. For $\\alpha = 1$ at least 6 data identifications are needed, at $\\alpha = 0.1$ significance at 0.05 is achieved, if at least 11 participants selected the data panel.}", fig.width = 8, fig.height = 5, out.width = "\\textwidth">>=
