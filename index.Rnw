...
\address[1]{\orgdiv{Org Division}, \orgname{Universty of Nebraska-Lincoln}, \orgaddress{\state{State name}, \country{Country name}}}

\address[2]{\orgdiv{Department of Mathematics}, \orgname{Iowa State University}, \orgaddress{\state{Iowa}, \country{United States}}}



\address[3]{\orgdiv{Org Division}, \orgname{Monash University}, \orgaddress{\state{State name}, \country{Australia}}}
...
\begin{figure}\label{fig:simplex}
\includegraphics[width=\textwidth]{figure/simplex-1.pdf}
\caption{Dirichlet distributed samples on the 2-dimensional simplex.}
\end{figure}
...
Clearly, the choice of $\alpha$ is critical. From a practical perspective, the number of null plots which are visually salient and thus likely to be selected by participants is a factor of the lineup design (zero, one, or two targets), null plot generation method, and possibly the form of the plot (aesthetics, geometric representations, scales). While the prior predictive distributions in \autoref{fig:prior-predictive} are illustrative, in practice, we do not usually have a good instinct for what a reasonable value of $\alpha$ would be for a particular lineup generation method. 

\svp{There are methods for maximum likelihood estimation of $\alpha$ \cite{sirt, minka}. 
For each lineup experiment, we could include one or two Rorshach lineups which would allow us to estimate $\alpha$ for the null plot generation method. 
...
Unfortunately, even in the best designed experiments, there are usually many panels with 0 selections, because people tend to find some feature to hone in on. 
As with many estimation methods, the MLE estimates of $\alpha$ are unstable when there are too many 0 values in the observed data. 
...
\svp{Instead of numerical estimation, in this paper, we propose a method for visual estimation of alpha which is not hampered by the prevalence of 0-count values.
This method is based on the assumption that during the creation of a null plot generating model (and viewing of subsequent lineups), the experimenter generally has an estimate of roughly how many null plots in a lineup tend to be visually interesting.}
...
prior_pred <- tibble(alpha = alphas,
                     plot_sels = purrr::map(alpha, sim_lineup_model, N = 100, k = 40)) %>%
  mutate(
    sel_ordered = purrr::map(plot_sels, ~apply(., 2, sort, decreasing = T)),
    sel_ordered_long = purrr::map(
      sel_ordered,
