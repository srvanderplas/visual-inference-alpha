...
When only one null plot in a Rorschach lineup is visually interesting, we encounter two problems: \dc{(1) $\hat\alpha$ cannot be estimated with precision (visually or numerically), because many different $\alpha$ values below $\alpha\approx 0.025$ could give rise to a situation where only one panel is visually interesting; (2) any one-target lineup may be confounded if the data plot replaces the interesting null plot.} %If there is only one interesting null panel in a Rorschach lineup, the equivalent one-target lineup would have between 1 and 2 interesting panels, so the number of interesting panels in the data plot would depend on whether the interesting null plot is the null replaced by a data plot.
