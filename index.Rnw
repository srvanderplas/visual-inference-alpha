...
We propose estimating $\alpha$ visually \dc{using} the number of null panels \dc{that} \svp{are $c$-interesting, for a $m$-panel lineup evaluated $K$ times}. \svp{A natural threshold for sufficient participant interest is $c = K/m$}; that is, setting the threshold $c$ to be greater than the expected number of selections under the Binomial model assumption that all panels have selection probability $1/m$. 
