...
As seen in \Cref{fig:vis-p-val-sensitivity-initial} the BB model converges  for $\alpha \rightarrow\infty$ asymptotically to the Binomial model (with $\theta = 1/m$) proposed in \citet{majumder2013validation}.
...
A natural threshold for sufficient participant interest is $c = K/m$; that is, setting the threshold $c$ to be equal to the expected number of selections under the specified BB model. %HH: Beta-binomial has expected value K/m
...
Using the DM model, computed visual $p$-values are more conservative than those previously computed using the Binomial model proposed in that work.
