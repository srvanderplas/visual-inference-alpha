...
l1 <- lineup(null_permute('y1'), df, n=5) %>%
  mutate(subject = "Subject 1")
l2 <- lineup(null_permute('y1'), df, n=5) %>%
  mutate(subject = "Subject 2")
l3 <- lineup(null_permute('y1'), df, n=5) %>%
  mutate(subject = "Subject 3")
...
<<example-plot, dependson = "example-setup", echo = F, fig.width = 5, fig.height = 5, out.width = ".95\\textwidth", fig.cap = "A lineup designed to test the utility of boxplots for detecting distributional differences. Selection counts are provided in light grey in the lower right corner of each panel which was selected at least once.">>=
