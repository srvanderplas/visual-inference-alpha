...
The expected number of panels selected more than $c$ times, $E[Z_c]$, is provided in \autoref{eqn:ev-panel}. The derivation of $E[Z_c]$ from \autoref{eqn:full-model-specification} is provided in \autoref{app:expected}.
...
% \hh{XXX suggestions for simplifying: 
% (1) let's define $c$-interestingness beforehand; (2) let's move all doubt about the number of null plot selections to the back and deal with them in one go. (3) let's give each one of the list items a header summarizing it. }
...
The probabilities $P(C_i > c)$ are derived in the previous section as marginal \dc{Beta-binomials}: \dc{Changed DBeta to Beta}
\begin{equation}
E[Z_c(\alpha)] = \frac{m}{ B(\alpha, (m-1)\alpha)} \cdot \sum_{x=c+1}^K \binom{K}{x} B(x+\alpha, K-x+(m-1)\alpha).
\end{equation}
