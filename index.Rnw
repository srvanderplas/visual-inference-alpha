...
\autoref{eqn:differentPanels} can be used to generate a more precise estimate of $\alpha$ with a secondary study consisting of one or more Rorschach lineups evaluated $K$ times each.
...
The numerical estimation approach does require each Rorschach lineup to have the same number of null plot evaluations, so that the experimentally determined average number of interesting panels is consistent and the same reference distribution can be used.
