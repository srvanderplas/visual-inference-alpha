...
\hh{Under the null hypthesis, the data panel is not distinguishable from the null plots. We can therefore model the picking probabilities $\bm\theta$ for the panels of a lineup }
...
Instead, we propose a method for \emph{visual} estimation of $\alpha$ which accommodates zero-count values and a numerical method for estimating $\alpha$ based on the results of Rorschach lineup based testing. %\dc{These are described here.}
