...
\subsection{\dc{Effect of} \texorpdfstring{$\alpha$}{alpha} on Lineup Scenarios}\label{sec:scenario-alpha}
...
\begin{align*}
f(\theta \mid \alpha) & = \svp{\frac{1}{B\left(\alpha, (m-1)\alpha\right)}}\cdot \theta^{\alpha - 1}(1-\theta)^{(m-1)\alpha - 1}\\[1em]
