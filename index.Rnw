...
\item Averate the results of ~10 simulations together to make the underlying trend more visible.
\item Determine cutoffs for ~5 bands indicating different levels of interest. In \autoref{fig:alpha-sim-curve}, the bands are approximately 2 panels in width; this ensures that 
...
\item Select an $\alpha$ value corresponding to the selected band. If the lineup contained 4-5 interesting null panels, we might use $\alpha = 0.075$. 
\end{enumerate}
...
band_labels <- tibble(
  y = c(1.5, 2.75, 4.5, 6.5, 8.5),
  x = min(alphas),
  label = c("1-2", "2-3", "4-5", "6-7", "8+ Interesting panels")
)
