...
\dc{grammar of graphics initially laid out by \citet{wilkinson:1999}, and further developed in \citet{hadley:2009}, which provides a functional mapping from variables (abstractly defined) into graphical elements of a plot.
...
For example, the scatter plots shown in the one target lineups in Figure \ref{fig:typesoflineups}b \svp{map} variable 1 to the $x$-axis and variable 2 to the $y$-axis, with the intent to study the association between the two variables.
...
The assumption is that each evaluator has to make a selection. \hh{In practice, evaluators get to make several selections -- these choices are incorporated into counts as fractions adding to 1. In case an evaluator makes no choice, all panel counts are increased by 1/m. For notational convenience, we will assume that all $c_i$ are integer counts. The results are immediately applicable to a non-integer number of selections, however.}
...
\svp{When participants select only a small subset of interesting null panels, there are naturally many panels that have no selections.}
...
\dc{The many zeroes} are even more pronounced when $\alpha$ values are small and only one or two null panels attract participant interest.
...
\dc{Instead,} \svp{in this section}, \dc{we} propose a method for \emph{visual} estimation of $\alpha$ \svp{which accommodates} \dc{zero-count values and} a \dc{numerical} method for estimating $\alpha$ based on the results of Rorschach lineup based testing. %\dc{These are described here.}
...
\svp{Using this definition,} we \svp{also} define the random variable $Z_c$, the number of panels which \svp{are $c$-interesting} in $K$ evaluations of an $m$-panel lineup.
...
\Cref{fig:prior-predictive} \dc{illustrates} that $\alpha$ is directly related to the number of panels that are selected as interesting by participants.
...
This is a more efficient use of participant time \svp{and experimenter resources}, as we do not have to ask \svp{participants} to evaluate two separate lineups to determine the visual $p$-value for a plot.
...
By controlling for the perceptual difficulty of a task using quantitative parameters, we can produce visual $p$-values that account for the \dc{demands of the} task  while still providing an assessment of the significance of the test statistic. 
