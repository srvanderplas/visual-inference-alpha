...
In order to test whether a chart shows a visually significant result, we can use the same \dc{machinery} used by randomization tests: construct a \dc{randomization} \hh{XXX not all nulls are  randomization based - how about "construct a method to generate data consistent with the null hypothesis, such as using randomization or drawing samples from a model "} \dc{ok} method consistent with the null hypothesis, generate many copies of the test statistic (in this case, the plot), and see where the \dc{observed} statistic falls in the distribution of artificially generated quantities. \dc{The conceptual framework was described in \citet{buja:2009}), who introduced two protocols: a Rorschach and a lineup. Figure \ref{fig:typesoflineups} illustrates these. In a Rorschach all plots are null plots, and the purpose is to purely digest the variation that migh occur even if the data is generated only by the null mechanism.}
...
In addition to different types of lineups, there are different \dc{ways that an analyst can run a lineup experiment.} These are detailed below and also illustrated in Figure \ref{fig:scenarios}.
