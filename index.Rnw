...
Using these methods, we could design our experiment to include one or two Rorshach lineups which would allow us to estimate $\alpha$ for the null plot generation method. 
...
\subsection{Visual Selection of \texorpdfstring{$\alpha$}{alpha}}
\svp{
...
\item Determine which band is most representative of the lineups in the experiment: Over many generated lineups, how many visually interesting null panels are there?
...
ggplot(pv, aes(x = alpha, y = p, group = factor(C))) +
  ...
  ggrepel::geom_label_repel(data = labels_data %>% filter(type=="is inconclusive"), aes(label = C, colour = type), hjust = 0, nudge_x = 0.05, size = 3, alpha = 0.8,
         min.segment.length= 0, label.padding = 0.1, direction = "y") +
   theme_bw() +
   geom_label(label = "# data picks (out of K = 30)", 
              aes(x = .010, y = 0.25), size = 3,
         data = labels_data[1,], hjust = 0, vjust = 1) +
   scale_colour_manual("Number of data picks ...",
                       values = c("grey70", "grey40", "red")) +
   theme(legend.position = c(.99, 0.99), legend.justification = c(1, 1))

@

% \hh{XXX we should include the S curve plot of alpha dependency on number of interesting plots on average. The discretization of alpha should also be included in that plot. The discussion doesn't necessarily fit here, but it's not the worst place.}
