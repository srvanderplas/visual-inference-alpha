...
Define $c = (c_1, ..., c_m)$ to be the counts corresponding to the set of $K$ evaluations, where $c_i$ is the number of times a participant selected panel $i$, \dc{where}
\hh{$0 \le c_i \le K$ for all $1 \le i \le K$ and $\sum_i c_i = K$. } \dc{(This assumes that evaluators must make a selection, and if they make more than one selection, their count is weighted to sum to 1. The derivations below accommodate fractional counts.)}

\svp{The Multinomial $(K, \theta)$ distribution}, \dc{with probability mass function given as:} 

\begin{align}\label{eqn:multinomial-pmf}
f(\bm{c}|K, \bm{\theta}) & = \frac{K!}{c_1! \cdots c_m!} \prod_{i=1}^m \theta_i^{c_i}
\end{align}

\noindent is a natural model for \svp{the counts $c_i$}, where $K$ describes the number of \hh{evaluations} and $\bm{\theta} = \theta_1, ..., \theta_m$ describes the probabilit\svp{y that each panel $i=1, ..., m$ is selected}.
%\svp{Its probability mass function is shown in \autoref{eqn:multinomial-pmf}}. 
\dc{The} panel selection probabilities $\bm{\theta}$ can be modelled using a Dirichlet distribution with concentration hyperparameter $\bm{\alpha} = (\alpha_1, ..., \alpha_m)$.
