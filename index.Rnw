...
\title{\dc{Statistical Significance Calculations for Several} Scenarios in Visual Inference\protect\thanks{This is an example for title footnote.}}
...
\dc{if the relationship between two variables is not adequately summarized by the numerical statistics} \svp{XXX Is that really the only use case for visual inference? As a backup if classic numerical inference fails?} 
we can gain more information by considering a simple scatter plot than we might obtain from an entire day of staring at the numbers. 

Statistical graphics \hh{allow us to} leverage the bandwidth of the visual system for implicit data analysis. \hh{B}ecause this analysis is implicit, we often assume \svp{these visual analyses} are not decisive in the same way that a hypothesis test is decisive.
\svp{G}enerally, graphics do not come with a significance threshold, and in many cases we do not explicitly declare the hypothesis we might be testing before viewing the \dc{data plot}.\svp{XXX this is more general than lineups: we haven't introduced lineups yet, so we are talking about more generic charts.}
However, there are 
\dc{emerging} \svp{XXX visual inference is a decade old... if it hasn't emerged by now, what are we waiting for?} 
formal protocols for \emph{visual inference}, \svp{using graphical displays to test hypotheses visually.} \svp{XXX this still seems a bit awkwardly worded} %through the} data visualization, \dc{loosely grouped into the label, visual inference}. %\svp{Too many uses of ``graphics" in a single paragraph}


Visual inference \dc{builds from considering data plots} to \dc{be} visual statistics: charts are, after all, a quantity computed from values in a sample of data. 

In order to test whether a chart shows a visually significant result, we can use the same \dc{machinery} used by randomization tests: 
(1) construct a method to generate data consistent with the null hypothesis, such as using randomization or drawing samples from a model consistent with the null hypothesis, (2) generate many copies of the test statistic (in this case, the plot), and (3) see where the \dc{observed} statistic falls in the distribution of artificially generated quantities.
\dc{The conceptual framework was described in \citet{buja:2009}), who introduced two protocols: a Rorschach and a lineup. Figure \ref{fig:typesoflineups} illustrates these.} In a Rorschach all plots are null plots, and the \hh{Rorschach's main} purpose is to assess the \hh{extent of visual} variation that occurs in data generated  by the null mechanism.
...
With numerical statistics, there is a natural ordering to \dc{the computed numerical quantities. With} plots, though, we must run each statistic through another \dc{process: evaluation} by the visual system, or a facsimile thereof
...
During this evaluation process, the user selects one or more plots from the lineup which are ``different" in some way. This choice  \dc{may or may not} be based on a feature under investigation by the test.
% TODO: explain different lineup scenarios here? Not sure if this is the best place for it, or if it's necessary at all

Typically, graphical tests utilize a service like Amazon Mechanical Turk~\citep{turk} to acquire multiple evaluations of the same lineup.
If multiple individuals select the plot generated from the data rather than the null plots, this counts as evidence that the visual statistic (the data plot) is indeed different from the null plots rendered from data generated by the null mechanism \dc{as described in \citet{majumder2013validation}}.

% TODO: add lineup here

\svp{\citet{majumder2013validation} also} \dc{proposed an approach for calculating a $p$-value from the results of lineup evaluations of multiple subjects that uses the binomial distribution.} \dc{This approach is simple, and a reasonably good approximation for assessing the significance of structure in a plot, but it fails to account for different ways of applying a lineup protocol for multiple observers.}
...
\svp{Leveraging the model, we discuss the correspondance between the model and the perceptual process and propose a method for estimation of the hyperparameters and a diagnostic procedure for null plot generation models.}
...
\dc{XXX Add comparison of binomial vs dirichlet p-values for same lineups later}
...
In addition to different types of lineups, there are different \dc{ways that an analyst can run a lineup experiment.} These are detailed below and also illustrated in \autoref{fig:scenarios}.
...
\subsection{Lineups as tests}\label{sec:tests}

\hh{XXX We need explicit hypotheses - that's the point of lineups, to make implicit looking at plots into explicit visual inference.}
...
\svp{The joint distribution of $\bf c$, $\bf \theta$ is then a Multinomial-Dirichlet mixture distribution. 
...
<<prior-predictive, echo = F, fig.cap = "Simulation (of size 50) of the number of panel picks $c_i$ (sorted by their frequency) for different values of $\\alpha$ for a fixed number $K=30$ of plot evaluations. For low values of $\\alpha$ plot selections are concentrated on only a few plots, while higher values of $\\alpha$ show a wider spread of selections among more plots. In all 50 simulated lineups with $\\alpha=0.001$ only one of the panels was chosen. For $\\alpha = 1$ five different panels were each chosen at least two times, and ten different panels were chosen at least once. ", fig.width = 8, fig.height = 4, out.width = "\\textwidth" >>=
...
ggplot() +
  ...
  scale_y_continuous("# panel was picked (in K=30 evaluations)")
...
The general formula for calculating a visual $p$-value under this model is:
\begin{align}\label{eqn:beta-binomial}
p\text{-value} = P(C\geq c_t) = \sum_{x = c_t}^{K} \binom{K}{x} \frac{1}{B(\alpha, (m-1)\alpha)}\cdot B(x+\alpha, K-x+(m-1)\alpha)
\end{align}
where $c_t$ is the number of times the data panel was picked in $K$  evaluations of the lineup. \hh{$B(., .)$ is the Beta function defined as:}
\begin{align*}
...
From \autoref{fig:prior-predictive}, we know that \svp{this phenomenon is} most consistent with $\alpha \ll 1$. 
...
In the theoretical Multinomial-Dirichlet model proposed in this paper, the number of panels which could be expected to be visually different in a generalized lineup is a function of $\alpha$, the hyperparameter in \autoref{eqn:dirichlet-pdf} and \autoref{eqn:full-model-specification} (a derivation of this expectation is provided in Appendix \ref{app:expected}).
...
\hh{While it is generally possible to estimate $\alpha$ \citep{sirt, minka} directly based on proportions observed from panel selections in a lineup, these maximum likelihood approaches fail because of the large number of observed zeroes (many panels are not selected in a lineup evaluation). This problem is even more pronounced when $\alpha$ values are small. }


There are methods to directly estimate $\alpha$ using a maximum likelihood approach \citep{sirt, minka} based on  proportions observed from panel selections in a lineup.
Using these methods, we could design  experiments to include additional Rorschach lineups to allow us to estimate $\alpha$ for the null plot generation method. 
...
However, as with many estimation methods, the maximum likelihood estimates of $\alpha$ are unstable when there are too many zeroes in the observed data. In the case of lineups, many panels are not selected. This problem is even more pronounced when $\alpha$ values are small.
\svp{Unfortunately, in most lineup scenarios, we are also operating in the region $\alpha \ll 1$; as a result, methods for estimation of $\alpha$ are most likely to fail in precisely the region of the parameter space where we typically operate.}


\section{Estimation of \texorpdfstring{$\alpha$}{alpha}}
...
Visual estimates of correlation and linear regression are known to differ systematically from the numerical estimates, but not in a problematic way; rather, visual estimates tend to discount the effect of outliers, producing a more robust estimate of the statistical quantity of interest.}
\svp{In this application, we expect that visual estimation of $\alpha$ based on the expected number of ``interesting" panels, derived from \autoref{eqn:full-model-specification}, will produce a more robust estimate of $\alpha$ than the numerically unstable maximum likelihood estimates.}

\svp{In a lineup experiment, we compare the selections of the data panel relative to the aggregate selections of the null panel, using the marginal Beta-Binomial model. 
While this makes the calculations much simpler, because there is no need to keep track of the locations and selections of individual null plots, it does throw away some information. 
...
\svp{
It is obvious that the mathematical quantity associated with `attract participant interest' would correspond to a threshold: if a panel is selected at least $c$ times out of $K$ total evaluations, it is interesting. 
Determining what the value of the threshold $c$ should be is another matter. 
One relatively simple heuristic would be to set $c > K/m$; that is, setting the threshold $c$ to be greater than the expected number of selections under the Binomial model assumption that all panels have selection probability $1/m$. 
}
...
\svp{
Let us start with a simulation using the Dirichlet-multinomial model in \autoref{eqn:full-model-specification}, and assuming that the lineup experiment we are planning to conduct has a standard ($m = 20$ panel, $m_0 = 19$ null plot) lineup with $K_0=30$ null panel selections out of $K$ evaluations. 
The heuristic we propose for \svp{ad-hoc} estimation of $\alpha$ is as follows:
}
\begin{enumerate}
\item Simulate, for each potential value of $\alpha$, the number of panels selected by a number of participants exceeding an interest threshold $c$ (in \autoref{fig:alpha-sim-curve}, $K_0/m_0 = 30/19 \approx 1.58 = c$). 
It may be helpful to average the results of several simulations together to reduce the visual noise and make the underlying trend more visible.
\hh{The simulation captures the variability around the expected number of `interesting' panels - i.e.\ panels that are selected at least $c$ times. 
The expected number of interesting panels is given as the sum of probabilities that each panel is selected more than $c$ times: $\sum_{i=1}^m P(C_i > c)$. See \autoref{eqn:differentPanels} in Appendix \ref{app:expected} for details.
Note that this simulation should use $m = m_0$, that is, the number of null panels in the lineup, rather than the total number of panels, and $K_0$, the number of null panel selections. 
}
\item Determine cutoffs for ~5 bands indicating different levels of interest, by segmenting the range of the \svp{expected number of panels with more than $c$ selections into approximately equal size bands (with the exception of the case where the expected number of panels selected is equal to one; this case will be handled separately). }
In \autoref{fig:alpha-sim-curve}, the bands are approximately 2 panels in width; for $K_0=30$ this provides a balance between level of detail and our ability to estimate the number of visually interesting null plots.

If only one null panel is selected more than $c$ times, and there are a reasonable number of null panel selections overall, this may be a sign that the null plot generation method is unsuitable. This possibility will be discussed in more detail below; for now, it is sufficient to know that this is why we have created a separate band for the case where only one null plot attracts significant participant interest.
\item Using the simulated selected panel counts, find the band which is most consistent with the number of null panels selected more than $c$ times in the lineup. For instance, if there were four null panels with more than $c$ selections, we would use a range of 4-5 panels. 
Use of a range instead of the single point estimate is intended to account for some of the sampling variability: the same individuals shown the same lineup tomorrow might select slightly different panels. 
In a simulation, it is easy to increase $N$ until the results no longer change; this is expensive and much more time consuming when using human evaluations. 
\item Using the simulated selected panel counts, determine a range of panel selections which is most consistent with the number of null panels selected more than $c$ times in the lineup. For instance, if there were four null panels with more than $c$ selections, we might use a range of 4-5 panels. Use of a range instead of the single point estimate is intended to account for some of the sampling variability: the same individuals shown the same lineup tomorrow might select slightly different panels. In a simulation, it is easy to increase $N$ until the results no longer change; this is expensive and much more time consuming when using human evaluations. 
\item Select an $\hat\alpha$ value corresponding to the selected band. \svp{For instance, if our pilot study showed that} the lineups typicaly contain 2-3 interesting null panels, we might use $\hat\alpha = 0.045$. 
% \item To generate a visual p-value, it may be helpful to create a sensitivity plot similar to \autoref{fig:vis-p-value-sensitivity2}; this plot shows the p-value generated by a range of $\hat\alpha$ values, for a lineup evaluated $K$ times. Each panel corresponds to a range of $\hat\alpha$ values generated when there are a certain number of interesting null plots. In most cases, it is likely that the number of data plot picks will result in a clear determination of significance; data plot picks highlighted in red result in an ambiguous result. 
\end{enumerate}

<<vis-p-val-sensitivity-initial2, fig.cap = "Sensitivity of visual p-value to selection of $\\alpha$ under the beta-binomial model, for a $m=20$ panel lineup with $K=30$ evaluations. Data picks which result in ambiguous results under the discretized bands of $\\alpha$ values are highlighted in red. At any particular $\\alpha$ level, there are only a few values which result in an inconclusive results.", fig.width = 8, fig.height = 5, out.width = "\\textwidth">>=
alphas <- 10^(seq(-3, 2, by = .01))
data_breaks <- 1:30
...
\svp{In \autoref{fig:vis-p-val-sensitivity-initial2}, we show the implications of the visual selection method for each band of $\alpha$ values in terms of the number of target plot selections necessary to achieve statistical significance at the $p=0.05$ level. 
Due to the discretization of the expected number of panels with $>c$ selections, each range of $\alpha$ values is ambiguous for one or more potential data panel selection counts; these values are shown in red, with labeled thresholds for nonsignificance and significance.
...
Alternately, the experimenter could use the method described in \autoref{sec:rorschach-est-alpha} to produce a more precise $\hat\alpha$ which would provide definitive results.
}
...
Note that if on average only one null panel is selected, the estimated $\hat\alpha$ is not reliable: \autoref{eqn:differentPanels} does not have the sensitivity to differentiate the results of a lineup with $\hat\alpha = 0.001$ from the results of a lineup with $\hat\alpha = 0.01$. 
...
In contrast to the reference distribution used in \autoref{sec:visual-est-alpha}, the reference distribution in this approach is one with $m$ null panels.
...
\subsection{Degenerate null generation methods}
...
<<example-plot, dependson = "example-setup", echo = F, fig.width = 5, fig.height = 5, fig.align="center", out.width = ".8\\textwidth", fig.cap = "A lineup designed to test the utility of boxplots for detecting distributional differences. Selection counts are provided in light grey in the lower right corner of each panel which was selected at least once.">>=
...
#ggplot(exdat2, aes(x = group, y = vals, fill = factor(group))) + geom_boxplot() + 
#  facet_wrap(~.sample) + 
#  theme_bw() + 
#  theme(axis.text = element_blank(), axis.title = element_blank(), axis.ticks = element_blank()) + 
#  scale_fill_discrete(guide = "none") #+ 
  # geom_text(aes(x = Inf, y = -Inf, label = n), nudge_x = -1, nudge_y = 1, hjust = 1.2, vjust = -0.2, inherit.aes = F, data = exdf, color = "grey40")
@
\svp{Under the \hh{Dirichlet model XXX we need a name or an equation for this XXX}, the first step \hh{is} to assess the expected number of null panels which  attract attention. 
Using a threshold of $c=1$,  six of 19 null plots \hh{were} selected, so we might expect somewhere between \hh{XXX let's run the simulation for $K = 22$ and $m = 19$; XXX for $\alpha$ it doesn't matter whether we have dimension 19 or 20 - I'll put that piece of math into the appendix} 6 and 7 null panels to be selected in a Rorschach lineup.
Ideally, we would generate several Rorschach lineups for this step, but for simplicity, we will work off of a single one-target lineup for this example; a corresponding Rorschach lineup is shown in \autoref{fig:example-plot}(b) but was not evaluated experimentally.}

<<example-alpha-curve, fig.cap = "Number of expected panels with more than one selection in the 22 null plot selections  for the lineup shown in \\autoref{fig:example-plot}(a).", fig.width = 8, fig.height = 4, out.width = "\\textwidth">>=
set.seed(2401072)

alphas <- 10^(seq(-2, 2, by = .01)) # seq(0.001, 0.5, by = .001)

number_panels <- function(alpha, c=0, m = 19, K=22) {
  x <- (c + 1):K
  summation <- choose(K, x) * beta(x + alpha, K - x + (m - 1)*alpha)
...
band_labels <- tibble(
  y = c(3, 5.75, 8.25, 10.5),
  x = min(alphas),
  label = c("2-4", "5-7", "7-9", "10+ Interesting panels")
)

ggplot() +
  ggtitle("Simulation based on K = 22 evaluations and m = 19 (null) plots") + 
  geom_polygon(aes(x = x, y = y, group = interaction(type, band)), data = bands, fill = "grey", alpha = 0.5) + 
...
\svp{XXX this still needs a lot of work, but the rough outline is at least there}
