...
\hh{XXX I'm just writing out a couple of ideas that we discussed on Sunday}
...
studies_sorted <- studies_sum %>%
  ...
  ungroup()
# 
# ggplot(studies_sorted, aes(x = sorted_panel, y = prop*30, group = pic_id)) + 
#   geom_line(alpha = .05) + facet_wrap(~study)
# 
# studies_sorted %>%
#   filter(study == "turk4") %>%
#   ggplot(aes(x = sorted_panel, y = prop*30, color = factor(test_param), group = pic_id)) + 
#   geom_line(alpha = .05) + facet_wrap(~study) + 
#   guides(color = guide_legend(override.aes = list(alpha = 1)))
# 
# studies_sorted %>%
#   filter(study == "turk4", obs_plot_location != response_no) %>%
#   ggplot(aes(x = sorted_panel, y = prop*30, color = factor(test_param), group = pic_id)) + 
#   geom_line(alpha = .05) + facet_wrap(~study) + 
#   guides(color = guide_legend(override.aes = list(alpha = 1)))
# 
# # studies_sorted %>%
# #   filter(study == "turk6") %>%
# #   mutate(test_param2 = str_remove_all(test_param, "^turk6_|ref$|\\d{1,}"),
# #          color = str_extract(str_remove_all(test_param, "turk6_"), "\\d{1,}")) %>%
# #   ggplot(aes(x = sorted_panel, y = prop*30, group = pic_id, color = color)) + 
# #   geom_line(alpha = .3) + facet_wrap(~test_param2) + ggtitle("turk6") + 
# #   guides(color = guide_legend(override.aes = list(alpha = 1)))
# 
# 
# ## Still working through the rest of these...I w
...
\hh{Depending on the scenario, different distributions hold under the null hypothesis. Should we include the proof for Binomial under Scenario 1 in the appendix?}

\section{Lineup Model Specification under Scenario 3}
...
<<example-plot, dependson = "example-setup", echo = F, fig.width = 5, fig.height = 5, fig.align="center", out.width = ".8\\textwidth", fig.cap = "A lineup designed to test the utility of boxplots for detecting distributional differences. Selection counts from a lineup evaluation are provided in light grey for each panel which was selected at least once. Panel 8 shows the data plot.">>=
...
@
\svp{Under the Dirichlet-Multinomial model in \autoref{eqn:full-model-specification}, the first step \hh{is} to assess the expected number of null panels which  attract attention. 
