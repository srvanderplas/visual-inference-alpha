...
\svp{Consider a lineup under} 
% Let us assume that we have $K$ evaluations of a lineup under
\hh{scenario 3, i.e.\ }
\svp{the same lineup containing $m$ panels}%\hh{a  lineup consisting of the same $m$ panels},
\svp{independently evaluated $K$ times.} % \hh{was shown to $K$ independent evaluators.}
\svp{Define} % \hh{Let us assume that }
\hh{$c = (c_1, ..., c_m)$} \svp{to be} %are 
\svp{the counts corresponding to the set of $K$ evaluations, where $c_i$ is the number of times a participant selected panel $i$.}
% \hh{the counts of that evaluation, where $c_i$ corresponds to panel $i$ being selected by $c_i$ of the $K$ evaluators. 
\hh{Obviously, $0 \le c_i \le K$ for all $1 \le i \le K$ and $\sum_i c_i = K$. }
...
\svp{The Multinomial $(K, \theta)$ distribution is } a natural model for \svp{the counts $c_i$}, where $K$ describes the number of \hh{evaluations} and $\bm{\theta} = \theta_1, ..., \theta_m$ describes the probabilit\svp{y that each panel $i=1, ..., m$ is selected}. 
\svp{Its probability mass function is shown in \autoref{eqn:multinomial-pmf}}.
%of each outcome $1, ..., m$, i.e.\  $\theta_i$ is the probability with which an evaluator chooses panel $i$. 
...
As the position of the panels within the lineup are random, we can use a symmetric Dirichlet distribution, with $\alpha_i = \alpha, i = 1, ..., m$, that is, the concentration hyperparameter is constant. \svp{This is necessary because we do not know where a specific panel will be placed when the lineup is initially generated.}
...
When evaluating lineups, we generally compare the number of target plot identifications with the aggregate number of null plot identifications \citep[see]{majumder2013validation}. 
...
In most studies, a \svp{lineup} might be evaluated between 10 and 30 times; with a $m=20$ \svp{panel} lineup, even $\alpha = 1$ can easily dominate the participant selection data.
...
While graphical illustrations of the $m$-dimensional dirichlet distribution are more difficult \svp{when $m>3$}, we can use simulation to assess the meaning of $\alpha$ as it relates to how many panels in a lineup attract participant attention. \autoref{fig:prior-predictive} shows simulated $c_i$ counts (sorted for visual clarity) for several values of $\alpha$ \svp{in a $m=20$ panel lineup}.
...
<<prior-predictive, echo = F, fig.cap = "Simulation of the number of panel picks $c_i$, sorted by  frequency, for different values of $\\alpha$ with $K=30$ evaluations. For low values of $\\alpha$, plot selections are concentrated on only a few plots, while higher values of $\\alpha$ show a wider spread of selections among more plots. In all 50 simulated lineups with $\\alpha=0.001$ only one of the panels was chosen. For $\\alpha = 1$ five different panels were each chosen at least two times, and ten different panels were chosen at least once. ", fig.width = 8, fig.height = 4, out.width = "\\textwidth" >>=
...
\subsection{Hyperparameters for Lineup Scenarios}\label{sec:scenario-alpha}
...
% \svp{Moreover, assessments of interesting features in plots are shared across participants and even across generated lineups. When one participant identifies a unique feature, other participants are likely to note the same feature; allowing participants to provide open-ended reasoning in lineup studies can be used to identify some of these insights.}
% \svp{This dependence has no equivalence in an equivalent numerical test: under randomization testing or resampling models, there is a single, pre-specified evaluation metric used to aggregate the data. 
% An individual sample (or the relationships between a collection of $m$ samples) would have no effect on the selection of the evaluation metric, because that metric is set before the samples are created.}
% \svp{One of the most powerful features of lineup-based inference is that it allows us to identify differences in visual statistics using a battery of perceptual evaluations simultaneously; }
\svp{The process of assessment of a representative baseline, identification of unique and visually interesting features, and development of a corresponding hypothesis occurs in all lineup evaluations.
When combined with the lineup administration scenario, however, there are implications for our understanding of the meaning of $\alpha$.}
...
Instead, because we are averaging over all panels which could be generated by the data and null models (as both the data plot and the null plots are exchanged every time), we can claim that $\theta = 1/m$ under $H_0$. In the Beta-Binomial mixture model, this corresponds to infinite $\alpha$. 
As shown in \autoref{fig:vis-p-val-sensitivity-initial}, when $\alpha \rightarrow\infty$, the Beta-Binomial model converges asymptotically to the Binomial model proposed in \citet{majumder2013validation}.}
...
The Dirichlet-Multinomial model provides this flexibility through the introduction of the hyperparameter $\alpha$.}

The general formula for calculating a visual $p$-value under \svp{the Beta-Binomial model appropriate for use in Scenario 3} is:
...
\svp{Using \autoref{eqn:beta-binomial}, we can still calculate a visual p-value, but this p-value is dependent on the hyperparameter $\alpha$.}
\svp{We know from past studies \citep{loy2016variations,vanderplas:2017} that only a few lineup panels attract attention, even if all of the panels in a lineup are null plots}\footnote{
\svp{In fact, it would make very little sense to use a Rorschach lineup under Scenario 1; with no shared information between different evaluations, there would be no reasonable way to aggregate the results.}}.
\svp{Combining this observation with} \autoref{fig:prior-predictive}, \svp{ we would expect that} $\alpha \ll 1$.

\svp{$\alpha$ would be expected to vary based on the null generating model, type of plot, and other aesthetic choices: all of these factors would affect the visual distinctiveness of the null and actual data.}
...
$\alpha$ also modulates the visual p-value calculated from \autoref{eqn:beta-binomial}: \svp{when $\alpha$ is low, it is likely that there are one or more visually distinctive null plots, making it difficult to attribute data panel selections to definitive visual differences betweeen the null and data plots.}
...
color_pal <- RColorBrewer::brewer.pal(length(data_breaks), "Paired")#[c(6, 5, 8, 7, 4, 3, 2, 1, 10, 9)]
...
ggplot(pv, aes(x = alpha, y = p, color = factor(C), group = factor(C))) +
  annotate("segment", x =115, xend = 115, y = -Inf, yend = .8, color = "grey40", alpha = .5) +
  annotate("text", x = 115, y = .8,
           label = "Binomial", vjust = 0, hjust = .75) +
  annotate("segment", x = 1, xend = 1, y = -Inf, yend = .8, color = "grey40", alpha = .5) +
  annotate("text", x = 1, y = 0.8,
           label = expression(paste("θ ~ Unif on m-1 simplex")), vjust = 0, hjust = .5) +
  geom_line(size = 0.75) +
...
\svp{Under Scenario 3, one consequence of our tendency to consider the same subset of interesting null panels is that the less interesting null panels generally have 0 selections.}
...
\subsection{Degenerate null generation methods}\label{sec:bad-null}
...
\section{Neutrality of Dirichlet distributions}
Let $\bm\theta = (\theta_1, ... \theta_m)$ with $\bm\theta \sim $Dir$(\alpha)$ for some real-valued $\alpha > 0$. 
The neutrality property of Dirichlet tells us that $\theta_m$ is independent of (normalized) random variable $\bm\theta^{-m} := (\frac{\theta_1}{1-\theta_m}, ..., \frac{\theta_{m-1}}{{1-\theta_m}})$.
