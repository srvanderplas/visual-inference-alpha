...
@
Using the mixture model proposed here, computed visual $p$-values will be more moderate than those computed using the approach proposed in \citet{majumder2013validation}.
As an example consider the lineup from a past experiment as shown in \autoref{fig:example-plot}. The responses from an evaluations were somewhat ambiguous: the data plot in panel 8 was selected most frequently by participants (14 times), but in total, null plots were selected more frequently than the target plot (22 times). In particular, the null plot in panel 4 attracted ten selections.
...
All of these values are still significant at the 0.05 level, however these values are not nearly as small as the $p$-value computed under the Binomial model.
