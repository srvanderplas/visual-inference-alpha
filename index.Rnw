...
Statistical graphics allow us to leverage the full bandwidth of the human visual system for implicit data analysis. Because this analysis is implicit, we often assume these visual analyses are not decisive in the same way that a hypothesis test is decisive.
...
In its visual version, a lineup is typically composed of $m-1$ ``null" plots (generated under the null hypothesis) and one data plot containing the observed data. 
...
% ---- Lineups as tests --------------------------------------------------------
The natural next step is to consider using these visual statistics to conduct hypothesis tests.
...
Occasionally, because there is some inductive reasoning required of the participant, the feature used to select the ``different" panel is not the feature under investigation by the experimenter (see \citet{vanderplas:2017} for a specific example of this phenomenon across multiple participants).
...
When $\alpha_1 = \alpha_2 = ... = \alpha_m$, the distribution is a symmetric Dirichlet distribution, $Dirichlet(\alpha)$, where $\alpha := \alpha\cdot 1_{1\times m}$.  As all null plots are generated using the same process, we model the selection probability $\bm\theta$ for a set of null plots using a symmetric Dirichlet distribution.
...
\Cref{fig:simplex} shows ternary plots~\citep{ggtern} of values simulated from a 3-dimensional Dirichlet distribution which illustrate the effect of $\alpha$ on the sampled $\bm\theta$; corresponding marginal Beta densities are shown in \Cref{fig:betas}.
...
<<betas, echo=FALSE, out.width='\\textwidth', fig.width=12, fig.height=4, fig.cap="Marginal Beta($\\alpha, 2\\alpha$) densities corresponding to the Dirichlet densities in \\Cref{fig:simplex}.">>=
p <- seq(0,1, by=0.005)
alpha <- c(1/3,1,3)
...
The right side of \Cref{fig:prior-predictive} shows probabilities of null plot selections from three previous studies. Each line corresponds to one lineup. The vertical striping and the relatively large variability is due to the limited number of evaluations of each lineup (usually a lineup is not evaluated more often than 30 times). Clearly, each experiment is associated with a different distribution of $\theta$ that has characteristics corresponding to different $\alpha$ values. However, all three studies suggest $\alpha$ values of less than 1 when comparing with the simulated distributions.


<<prior-predictive, echo = F, fig.cap = "Simulation of the panel selection probabilities $\\theta_i$, sorted, for different values of $\\alpha$ (left). In the right column, panel selection probabilities estimated from lineups used in past visual inference experiments; these probabilities are calculated from lineups with finite evaluations, so the calculated probabilities are not continuous. For low values of $\\alpha$, plot selections are concentrated on only a few plots, while higher values of $\\alpha$ show a wider spread of selections among more plots. The included selection probabilities from previous studies \\citep{vanderplas_hofmann_cook_2020} show that not all panels in each lineup are selected, even when evaluated multiple times; it is also clear that the selection probabilities are more concentrated in the first study, where in some lineups only one panel was selected; in the third study, at least 4 panels were selected in every lineup. The studies selected differed in the specific graphical forms tested, the underlying randomization models used, and in experimental design; however, all three studies produce panel selection probabilities which behave similarly to the Dirichlet simulations for different values of $\\alpha$.", fig.width = 8, fig.height = 6, out.width = "\\textwidth" >>=
...
studies_sum <- readr::read_csv("data/turk-studies-summary.csv")
...
Alternatively, $L$ sets of lineups are shown to $K > L$ individuals.
...
In this scenario, lineup evaluations by independent viewers are not independent because the viewers are evaluating the same combination of  null plots and  data.
...
Consider a lineup experiment under scenario 3, i.e.\  the same lineup containing $m$ panels, independently evaluated $K$ times.
...
Under the null hypthesis, the data panel is not distinguishable from the null plots. We can therefore model the picking probabilities $\bm\theta$ for the panels of a lineup 
%We do not know where a specific panel will be placed when the lineup is initially generated, so we  % HH: \hh{XXXXXXXXXXXXXXXXXX the assumption of a symmetric dirichlet for a lineup is only correct under the null hypothesis.
%It does not have anything to do with random placement. Please don't delete again.}
using a symmetric  Dirichlet distribution, with $\alpha_i = \alpha, i = 1, ..., m$.  
...
\bm{c} \mid \bm{\theta} & \sim Multinomial(K, \bm\theta)
\end{split}\end{align}
...
We can conduct a test of the hypothesis that the Dirichlet distribution is symmetric, that is, whether our observed panel selection counts are consistent with a single Dirichlet parameter $\alpha$. This hypothesis implies that while not all panels are equally likely to be selected, there is no a priori belief that the panels systematically differ.
...
However, that approach can be considered to be equivalent to a special case of the marginal distribution of $c_i$ under the Dirichlet Multinomial model 
For a total of $K$ evaluations, and $0 \le c_i \le K$ target plot evaluations, the marginal model simplifies to a Beta-Binomial:

\begin{align}\begin{split}\label{eqn:marginal-model-specification}
\theta_t \mid \alpha &\sim Beta(\alpha, (m-1)\alpha) \\
C_i \mid \theta_i & \sim Binomial(K, \theta_i),
\end{split}\end{align}
...
As seen in \Cref{fig:vis-p-val-sensitivity-initial} the Beta Binomial model converges  for $\alpha \rightarrow\infty$ asymptotically to the Binomial model (with $\theta = 1/m$) proposed in \citet{majumder2013validation}; see \autoref{eq:binolimit} for a derivation.

<<vis-p-val-sensitivity-initial, fig.cap = "Sensitivity of visual p-values to selection of $\\alpha$ under the marginal beta-binomial model.  Corresponding values for the binomial model are shown on the right side of the plot; as $\\alpha \\rightarrow\\infty$, the beta-binomial $p$-values converge to the binomial model $p$-value. For different values of $\\alpha$ different number of data panel identifications are needed to achieve significance: 5 or more identifications are needed for significance at a level of 0.05 under the Binomial model. For $\\alpha = 1$ at least 6 data identifications are needed, at $\\alpha = 0.1$ significance at 0.05 is achieved, if at least 11 participants selected the data panel.", fig.width = 8, fig.height = 5, out.width = "\\textwidth">>=

alphas <- 10^(seq(-3, 2, by = .01))
data_breaks <- c(1:5, 6, 8, 10, 11, 15)
#data_breaks <- c(1, 2, 4, 5, 7, 10, 15, 20, 25, 30)
K <- 30
...
ggplot(pv, aes(x = alpha, y = p, color = factor(C), group = factor(C))) +
  geom_vline(xintercept = 1, colour = "grey40", alpha = 0.5) +
  geom_vline(xintercept = 150, colour = "grey40", alpha = 0.5) +
  annotate("label", x = 150, y = 1,
           label = "Binom (K,1/m)", vjust = .5, hjust = .75, 
           fill="white", label.size=0, alpha = 0.75) +
  annotate("label", x = 1, y = 1,
           label = expression(paste("θ ~ Uniform on m-1 simplex")), 
           vjust = .5, hjust = .5, alpha = 0.75, fill="white", label.size=0) +
  geom_line(size = 0.75) +
  geom_point(aes(x = 150, y = p, color = factor(C), shape = "Binomial\np-value"), data = pv2) +
  scale_y_sqrt(expression(paste("Visual p-value: P(C ≥ ",c[t],")")), breaks=c(0.0, 0.01, 0.05, 0.1, 0.2, 0.4, 0.6, 0.8), labels=c(0.0, 0.01, 0.05, 0.1, 0.2, 0.4, 0.6, 0.8)) +
  scale_x_log10(expression(alpha), breaks = breaks, labels = labels, minor_breaks = minor.breaks) +
  scale_color_manual(expression(paste("# Data Panel\nIdentifications ",c[t])), values = color_pal) +
  scale_shape_discrete("") +
  geom_hline(yintercept = 0.05, color = "grey40") +
  guides(color = guide_legend(override.aes = list(shape = NA), order = 1),
         shape = "none") +
  theme_bw() +
  theme(legend.position = c(0, .95), legend.justification = c(0, 1), legend.background = element_blank(), legend.box = "horizontal") +
  ggtitle("K = 30 Evaluations") 
@


Under Scenario 3, however, each participant evaluates the same lineup, with the same data  and null plots.
In this scenario, we have enough information that we can model the $\theta_i$ across different lineup evaluations.
Because the perceptual mechanisms which determine visual interest are shared across participants, and the same plots are used, we must allow $\theta_i$ to vary.
The Dirichlet Multinomial model (\Cref{eqn:full-model-specification}) provides this flexibility through the introduction of the parameter $\alpha$.
The general formula for calculating a visual $p$-value under the Beta Binomial model appropriate for use in Scenario 3 is:
...
We know from past studies \citep{hofmann2012graphical,zhaoMindReadingUsing2013,roychowdhury:2013,tengfei:2013,loyDiagnosticToolsHierarchical2013,majumderHumanFactorsInfluencing2014,loy:2015,loyAreYouNormal2015,loy2016variations,vanderplas:2017,loyModelChoiceDiagnostics2017} that only a few lineup panels attract attention, even if all of the panels in a lineup are null plots.
Combining this observation with \Cref{fig:prior-predictive}, we would expect that $\alpha \ll 1$.

We  expect the value of $\alpha$ to depend on several factors: the null generating model and the type of plot, and other aesthetic choices,  all of which  affect the visual distinctiveness of the null and actual data.
...
The selected $\alpha$ modulates the calculated visual $p$-value, as shown in \Cref{fig:vis-p-val-sensitivity-initial}: when $\alpha$ is low, there are likely a few visually quite  distinctive null plots drawing the attention away from the data. This makes it difficult to attribute data panel selections to definitive visual differences between the null and data plots.
...
When participants select only a  subset of interesting null panels, there are naturally many panels that have no selections.
...
Instead, we propose a method for \emph{visual} estimation of $\alpha$ which accommodates zero-count values and a numerical method for estimating $\alpha$ based on the results of Rorschach lineup based testing. %
...
Using this definition, we also define random variable $Z_c$, the number of panels which are $c$-interesting in $K$ evaluations of an $m$-panel lineup.
The expected number of panels selected at least $c$ times, $E[Z_c]$, is calculated as:
...
\end{equation}
A derivation of $E[Z_c]$ from \Cref{eqn:full-model-specification} is provided in \Cref{app:expected}.
...
Unfortunately, because we are usually dealing with relatively small values of $K$ and $m$, even when we normalize counts, we still deal with a set of discrete quantities.


In a lineup experiment, we typically compare the selections of the data panel relative to the \emph{aggregate} selections of null panels, using the marginal Beta Binomial model.
While this makes the calculations much simpler, because there is no need to keep track of the locations and selections of individual null plots, it does disregard some information; namely, the distribution of participant interest in the null panels.
...
The light grey horizontal bands in  \Cref{fig:alpha-sim-curve} mark different levels of $c$-interesting null panels. We consider situations with 2-3, 4-5, 6-7, and 8+  null panels that were selected at least once. The corresponding values for $\alpha$ are shown in the connected vertical bands.
If only one null panel is selected more than $c$ times, and there are a reasonable number of null panel selections overall, this may be a sign that the null plot generation method is unsuitable.  A discussion of this case is deferred to \Cref{sec:bad-null}.

We use \Cref{fig:alpha-sim-curve}  for visually estimating $\alpha$ by executing a reverse lookup of the rate parameter based on the number of interesting null panels.


A natural threshold for sufficient participant interest is $c = K/m$; that is, setting the threshold $c$ to be equal to the expected number of selections under the specified Beta-binomial model. %HH: Beta-binomial has expected value K/m
...
sim_interesting_panels <- function(c = m/K, N_points = 5, m = 19, K = 30, point_avg_sims = 10, alphas = alphas) {
  ...
  df <- tibble(alpha = alphas,
               plot_sels = purrr::map(alpha, rVis,
                                      N = N_points*point_avg_sims, K = K, m = m),
...
The proposed visual estimation process is conducted using a simulation from the Dirichlet Multinomial model in \Cref{eqn:full-model-specification}.
Furthermore, we will assume for this demonstration that the lineup experiment we are planning to conduct has a standard size lineup ($m = 20$ panel, $m_0 = 19$ null plots)  with $K_0=30$ null panel selections out of $K=40$ evaluations.

\begin{description}
\item[Identify the number of $c$-interesting null panels in the lineup of interest.] Consider \Cref{fig:alpha-sim-curve}. Generally, this figure needs to be adjusted to match the number of null panels,   $m_0$,  and the corresponding number of null panel selections, $K_0$. The \texttt{estimate\_alpha\_visual()} function in the \texttt{vinference} package can be used to create this plot given values of $c$, $m_0$, and $K_0$ \citep{vinference,R}.
...
The inexpensive, conservative approach is to declare any inconclusive results to be non-significant, in effect using the smallest $\alpha$ value corresponding to the approximate number of panels selected.
Alternately, the experimenter could use the method of using additional Rorschach lineups as described in \Cref{sec:rorschach-est-alpha} to produce a more precise $\hat\alpha$ which would provide definitive results, at the cost of conducting a secondary study.
...
Another interpretation of an extremely low $\hat\alpha$ value is to cast  suspicion on the null generating process. Null generating methods should not inadvertently generate highly variable numbers of interesting nulls; rather one expects them to be uniformly dull.
...
Using the Dirichlet Multinomial model, computed visual $p$-values are more conservative than those previously computed using the Binomial model proposed in that work.
...
This suggests that $p$-values generated using the Beta Binomial mixture model are better calibrated than those generated by the Binomial model.
...
The proposed Dirichlet-Multinomial model must be calibrated using a parameter $\alpha$ which describes the number of null panels in a lineup which are visually interesting. 
...
\subsection*{Supporting Information} The code and data necessary to reproduce this article are available at \url{https://github.com/srvanderplas/visual-inference-alpha}.

The data that support the findings of this study are also openly available in figshare at \url{https://doi.org/10.6084/m9.figshare.12894932}, reference number 12894932.
...
% 
% \begin{biography}{\includegraphics[width=60pt,height=70pt,draft]{example-image-a}
% }{\textbf{Dianne Cook} is Professor of Business Analytics in the Department of Econometrics and Business Statistics at Monash University. She received her PhD in Statistics from Rutgers University in 1993. She is a Fellow of the ASA.}\end{biography}
% 
% \begin{biography}{\includegraphics[width=60pt,height=70pt,draft]{example-image-a}
% }{\textbf{Heike Hofmann} is a Professor in the Department of Statistics  and Interim Professor in Charge of the Data Science Program at Iowa State University. She received her PhD from the University of Augsburg in 2000. She is a Fellow of the ASA.}
% \end{biography}

\appendix
\section{Visual \texorpdfstring{$p$}{p}-value distribution}\label{app:pvalue}
Assume we have a lineup of size $m$ with $K$ evaluations resulting in $c_t$ target plot evaluations. We defined the Beta Binomial model in \Cref{eqn:marginal-model-specification} leading to densities given as:
...
Now we use that $C \mid \theta \sim$ Binom$_{K, \theta}$ and $\theta \sim$ Beta$_{\alpha, (m-1)\alpha}$:
...
Note that for large values of $\alpha$ the densities converge in distribution to the Binomial with parameters $K$ and probability of success $p = 1/m$: 
the $\Gamma$ function can be written as a telescopic product, i.e.\ $\Gamma(x + \alpha) = \prod_{i=0}^{x-1}(i + \alpha) \Gamma(\alpha)$ for any non-negative integer $x \ge 0$. 
Therefore the probability for  $x$ target detections becomes
\begin{eqnarray*}
P(C = x) &=& {K \choose x} \frac{\Gamma(m\alpha)}{\Gamma(K+m\alpha)} \cdot  \frac{\Gamma((m-1)\alpha + K-x)}{\Gamma((m-1)\alpha)} \cdot \frac{\Gamma(\alpha+x)}{\Gamma(\alpha)}  \\
...
\prod_{i=0}^{K-x-1} \underbrace{\frac{(m-1)\alpha + i}{ m\alpha + i+x }}_{\to \frac{m-1}{m} \text{ for } \alpha \to \infty } \\\label{eq:binolimit}
&=& {K \choose x} \left(\frac{1}{m}\right)^x \left(1 - \frac{1}{m}\right)^{K-x}
\end{eqnarray}


\section{Scenario 1 is Binomial}\label{app:binomial}

Under Scenario 1 a lineup is shown to only one participant. Let $Z_i$ be the binary variable capturing whether lineup $i$ resulted in a data pick.
Then $C$, the number of data picks in $K$ evaluations of $K$ lineups is given as the sum of $K$ independent evaluations $Z_i$, i.e.\ $C = Z_1 + ... + Z_k$ .
Assuming (independent) Dirichlet samples for each lineup, assume $\theta_i$ is the probability (under the null hypothesis) to pick the data plot. That is, each evaluation is a Bernoulli trial with success probability $\theta_i$.
Then $C \mid \theta_1, ..., \theta_K \sim$  Poisson-Binomial distribution: 
\[
P(C = c \mid \theta_1, ..., \theta_K) = \sum_{A \in A_c} \prod_{i \in A} \theta_i \prod_{j \notin A} (1-\theta_j)
\]
where $A_c$ is the set of all subsets of $c$ integers that can be selected from $\{1,2,3,...,K\}$.

For the distribution of $C$ therefore holds:
\begin{eqnarray*}
P(C = c) &=&  \sum_{A \in A_c} \prod_{i \in A}  \underbrace{\int_{0}^1 \theta_i f(\theta_i) d\theta_i}_{E[\theta_i]} \cdot \prod_{j \notin A} \underbrace{\int_{0}^1 (1-\theta_j) f(\theta_j) d\theta_j}_{1- E[\theta_j]} = \\
  &=& \sum_{A \in A_c} \prod_{i \in A} \frac{1}{m} \cdot \prod_{j \notin A} (1 - \frac{1}{m}) = \\
  &=& {K \choose c} \left(\frac{1}{m}\right)^c \left(1-\frac{1}{m}\right)^{K-c} 
\end{eqnarray*}
This shows that under Scenario 1 the null distribution of the number of data picks simplifies to a Binomial $(K, \frac{1}{m})$ distribution. % rearrange because we use B(,) for beta function below
\section{Expected number of panels picked}\label{app:expected}
Define $C = (C_1, ..., C_m) \sim \text{Mult}_{K, \theta}$ to be a (simulated) lineup that is evaluated $K$ times, and $\theta = (\theta_1, ..., \theta_m) \sim \text{Dir}_\alpha = (\alpha, ..., \alpha)$ with $\sum_i \theta_i = 1$.
...
\section{Partitions of Dirichlet distributions}\label{neutrality}
...
Further, this implies that the conditional distribution $\bm\theta^{-m} \mid \theta_m \sim $ Dir$(\alpha)$. In particular, the density of the joint distribution of $\theta_m$ and $\theta^{-m}$ factors as a Gamma density for $\theta_m$ and an $m-1$-dimensional symmetric Dirichlet density with the same parameter $\alpha$ for $\theta^{-m}$.
...
<<rename-figures, include = F>>=
# This only works if all figures are generated in code chunks...
chunks <- readLines("index.Rnw")

chunks <- chunks[str_detect(chunks, "<<.*?>>=")] %>% str_extract("<<[\\w-]{1,},") %>% str_remove_all("<<|,") %>% factor(., levels = ., ordered = T)
fig_table <- right_join(
  tibble(factor = chunks, name = paste0(as.character(chunks), "-1")),
  tibble(file = list.files("figure/", full.names = T), name = basename(file) %>% str_remove("\\..*$")) %>%
    filter(!str_detect(name, "vanderplas"))
) %>%
  arrange(factor) %>%
  mutate(fig = factor(name, levels = unique(name), ordered = T), fignum = as.numeric(fig), ext = str_remove(basename(file), name) %>% str_remove("^\\."))

file.copy(fig_table$file, paste0("figure/vanderplas-figure", fig_table$fignum, ".", fig_table$ext), overwrite = T)
@
\end{document}
