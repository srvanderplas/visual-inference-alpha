...
As we are primarily interested in the average number of panels which are ``interesting" as indicated by selection by multiple people, we will average the results of 10 simulations so that the underlying trend is more visible. 
