...
\newcommand{\subfloat}[2][need a sub-caption]{\subcaptionbox{\normalsize #1}{#2}}
% --- Editing ------------------------------------------------------------------
...
\svp{Essentially, statistical graphics} leverage the bandwidth of the visual system for implicit data analysis, and because this analysis is implicit, we often assume \svp{these visual analyses} are not decisive in the same way that a hypothesis test is decisive.
\svp{G}enerally, graphics do not come with a significance threshold, and \dc{typically,} we do not explicitly \dc{declare} the hypothesis we might be testing before viewing the \dc{data plot}.
\svp{However, there are} \dc{emerging} \svp{formal protocols for conducting inference through the} data visualization, \dc{loosely grouped into the label, visual inference}. %\svp{Too many uses of ``graphics" in a single paragraph}
