...
\svp{This model is more flexible than the binomial model used in \citet{majumder2013validation}, allowing us to better represent the perceptual process of lineup evaluation.}
...
\svp{Leveraging the proposed model, we discuss the correspondance between the model and the perceptual process and propose a method for estimation of the hyperparameters and diagnostic procedure for null plot generation models.}
