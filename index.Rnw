...
In order to test whether a chart shows a visually significant result, we can use the same \dc{machinery} used by randomization tests: construct a method to generate data consistent with the null hypothesis, such as using randomization or drawing samples from a model consistent with the null hypothesis, generate many copies of the test statistic (in this case, the plot), and see where the \dc{observed} statistic falls in the distribution of artificially generated quantities. \dc{The conceptual framework was described in \citet{buja:2009}), who introduced two protocols: a Rorschach and a lineup. Figure \ref{fig:typesoflineups} illustrates these.} In a Rorschach all plots are null plots, and the \hh{Rorschach's main} purpose is to assess the \hh{extent of visual} variation that occurs in data generated  by the null mechanism.
...
\hh{In its visual version, } lineups are typically composed of $m-1$ ``null" plots (generated under the null hypothesis) and one data plot containing the \dc{observed} data. \hh{This lineup is also called a  one target lineup; two-target lineups have been introduced in  \citet{vanderplas:2017}. Two-target lineups} can be used to test the visual salience of two competing effects. Technically, \hh{this makes} the Rorschach \hh{protocol a zero-target} lineup.
