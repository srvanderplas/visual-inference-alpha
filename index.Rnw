...
ggplot(pv, aes(x = alpha, y = p, color = factor(C), group = factor(C))) +
  ...
  annotate("text", x = 150, y = .8,
           label = "Binom (K,1/m)", vjust = 0, hjust = .75) +
  ...
  scale_y_continuous(expression(paste("Visual p-value: P(C ≥ ",c[t],")"))) +
...
the $\Gamma$ function can be written as a telescopic product, i.e.\ $\Gamma(x + \alpha) = \prod_{i=0}^{x-1}(x + \alpha) \Gamma(\alpha)$ for any non-negative integer $x \ge 0$. 
