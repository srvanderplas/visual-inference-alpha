...
\hh{The simulation captures the variability around the expected number of `interesting' panels}\svp{.}\hh{
...
\svp{Note that this simulation should use $m = m_0$, that is, the number of null panels in the lineup, rather than the total number of panels, and $K_0$, the number of null panel selections. }
