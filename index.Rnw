...
\hh{\autoref{fig:prior-predictive} illustrates this relationship for the example of $K=30$ evaluations of 19 null panels. . 
The blue line corresponds to the expected number of 1-interesting null panels $Z_1$ given in \autoref{eqn:ev-panel}.
...
We propose estimating $\alpha$ visually \dc{using} the number of \svp{$c$-interesting} null panels \svp{in an $m$-panel lineup evaluated $K$ times}. \svp{A natural threshold for sufficient participant interest is $c = K/m$}; that is, setting the threshold $c$ to be greater than the expected number of selections under the Binomial model assumption that all panels have selection probability $1/m$. 
...
\begin{description}
\item [Divide \dc{null panel selection activity into four levels}] \dc{as shown in \autoref{fig:alpha-sim-curve}}, using a visual display with both simulated values and the expected value \svp{computed using \autoref{eqn:ev-panel}. Note that $Z_c=1$ should be treated as a separate level.} 
\svp{For a $20$ panel lineup with $K_0=30$ null panel selections,} these levels are 2-3, 4-5, 6-7, and 8+ panels.

{\it Aside:} If only one null panel is selected more than $c$ times, and there are a reasonable number of null panel selections overall, this may be a sign that the null plot generation method is unsuitable.  The case where only one null panel is selected is handled separately; discussion of this case is deferred to \autoref{sec:bad-null}. For now, it is sufficient to know that this is why we have created a separate band for the case where only one null plot attracts significant participant interest.

\item [\svp{Locate the y-axis range corresponding to the observed data}] Using the simulated selected panel counts, \dc{on the vertical axis locate} the band which is most consistent with the number of null panels selected \dc{with} more than $c$ times in the lineup. For instance, if there were three null panels with more than $c$ selections, we would use a range of 2-3 panels.
