...
Assuming (independent) Dirichlet samples for each lineup, assume $\theta_i$ is the probability (under the null hypothesis) to pick the data plot. \svp{That is, each evaluation is a Bernoulli trial with success probability $\theta_i$.}
