...
\dc{Another interpretation of an extremely low $\hat\alpha$ value is to cast  suspicion on the null generating process. Null generating methods shouldn't inadvertently generate highly variable numbers of interesting nulls, rather one eexpects them to be uniformly dull.}
