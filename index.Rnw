...
<<alpha-sim-curvea, cache = F, fig.width = 8, fig.height = 4, out.width = "\\textwidth", fig.cap = "Average number of panels selected more than once for a range of $\\alpha$ values. Each point represents 10 simulations of lineups with $K=30$ evaluations. The line in red shows the expected number of panels selected more than once as given in \\autoref{eqn:interesting}. Bands are shown in alternating grey and white corresponding to a discretized heuristic for selection of $\\alpha$ when $K=30$.">>=
set.seed(2401072)
...
ggplot() +
  ...
  scale_y_continuous(name = "Average number of panels with >1 selection", breaks = 1:100) +
   geom_line(aes(x = alpha, y = avg_interesting), data = dframe, colour = "red")
...
\section{Expected number of panels picked}\label{appendixB}
