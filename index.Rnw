...
intercept <- mean(turk6df$n - .5) # Take off 0.5 to correct for the fact that these are data lineups, not Rorschach lineups
...
<<example-plot, dependson = "example-setup", echo = F, fig.width = 5, fig.height = 4.25, out.width = ".6\\textwidth", fig.cap = "A lineup designed to test the utility of boxplots for detecting distributional differences. This lineup was evaluated 36 times; selection counts are provided in light grey in the lower right corner of each panel which was selected at least once." >>=
