...
prior_pred_mean <- prior_pred %>%
  mutate(rep2 = rep %% 10) %>%
   ...
   summarize(n_sel_plots = sum(sels > interest_threshold)) %>%
   group_by(alpha, rep2) %>%
...
Under the model proposed in \citet{majumder2013validation}, this plot would produce a visual p-value of \Sexpr{old_vispval}, which is extremely small given that there were more null plot selections than data plot selections.}
...
@
\svp{Under the proposed method, our first step would be to assess the expected number of null panels which would attract attention. 
Using a threshold of $c=1$, we have 6 of 19 null plots selected, so we might expect somewhere between 6 and 7 null panels to be selected in a Rorschach lineup. 
...
<<example-alpha-curve, fig.cap = "Generated discretized heuristic plot for the lineup shown in \\autoref{fig:example-plot}(a).", fig.width = 8, fig.height = 4, out.width = "\\textwidth">>=
set.seed(2401072)

sim_lineup_model <- function(alpha, m = 20, K = 36, N = 50) {
  theta <- gtools::rdirichlet(1, rep(alpha, m))
  sels <- rmultinom(N, size = K, prob = theta)
  sels
}
...
number_panels <- function(alpha, c=1, m = 20, K=36) {
  x <- (c + 1):K
  summation <- choose(K, x) * beta(x + alpha, K - x + (m - 1)*alpha)
...
ggplot() +
  ...
  geom_text(aes(x = x, y = y, label = label), data = band_labels, hjust = 0, color = "grey30") + 
  coord_cartesian(xlim = range(alphas)) + 
  scale_y_continuous(name = "Average number of panels with >1 selection, with 36 evaluations", breaks = 1:100)
@
