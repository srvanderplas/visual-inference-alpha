...
\hh{Let $C_t$ be the random variable capturing the number of data (target) selections, $0 \le C_t \le K$. The {\bm visual $p$-value} is defined as the probability that we see at least $c_t$ evaluations in $K$ independent lineup evaluations:  }
\begin{equation}\label{pvalue}
\text{visual } p-\txt{value} = P(C_t \ge c_t)
\end{equation}
