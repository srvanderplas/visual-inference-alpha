...
In order to test whether a chart shows a visually significant result, we can use the same \dc{machinery} used by randomization tests: construct a \dc{randomization} method consistent with the null hypothesis, generate many copies of the test statistic (in this case, the plot), and see where the \dc{observed} statistic falls in the distribution of artificially generated quantities (\dc{see the conceptual framework described in} \citet{buja:2009}). 
An assembly of several null plots with a target (or data) plot is called a \emph{lineup}, after the criminal procedure of the same name. Typically, lineups are composed of 19 ``null" plots (generated under the null hypothesis) and one data plot containing the \dc{observed} data.
