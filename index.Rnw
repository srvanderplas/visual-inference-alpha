...
exp_average <- alphas %>% sapply(FUN=number_panels)
...
dframe <- data.frame(alpha = alphas, avg_interesting = exp_average, stringsAsFactors = FALSE)
...
ggplot() +
  ...
  scale_y_continuous(name = "Average number of panels with >1 selection", breaks = 1:100) +
   geom_line(aes(x = alpha, y = i), data = dframe, colour = "red")
...
The probabilities $P(X_i > c)$ are derived in the previous section as marginal DBeta-binomials:
